...
public sealed class KugouLyricsProvider(LyricsHttpClient http) : ILyricsProvider
{
    ...
    public async Task<LyricsDocument> QueryAsync(LyricsQuery query, CancellationToken cancellationToken)
    {
        ...
        if (best is null)
        {
            ...
            { Item = item, Score = LyricsMatcher.Score(query, Text(item, "SongName"), Text(item, "SingerName"), Text(item, "AlbumName"), Number(item, "Duration") / 1000) })
    ...
    private static Candidate? Choose(System.Text.Json.JsonElement root, LyricsQuery query)
    {
