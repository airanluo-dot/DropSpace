...
public sealed class LyricsHttpClient(HttpClient client)
{
    ...
    public async Task<JsonDocument> GetAsync(string url, CancellationToken token, string? referer = null)
    {
        ...
        if (finalUri is null || finalUri.Scheme != "https" || !Hosts.Contains(finalUri.Host)) throw new InvalidDataException("Untrusted lyrics response.");
        ...
        var json = Encoding.UTF8.GetString(buffer.GetBuffer(), 0, checked((int)buffer.Length)).Trim();
    ...
    public static string Escape(string text) => Uri.EscapeDataString(text[..Math.Min(text.Length, 2_048)]);
    public static string Text(JsonElement element, string property) => element.TryGetProperty(property, out var value)
    ...
    public static double Number(JsonElement element, string property) => element.TryGetProperty(property, out var value) && value.ValueKind == JsonValueKind.Number && value.TryGetDouble(out var number) && double.IsFinite(number) ? number : 0;
    public static IEnumerable<JsonElement> Array(JsonElement element, params string[] path)
    {
        foreach (var name in path) if (!element.TryGetProperty(name, out element)) return [];
        return element.ValueKind == JsonValueKind.Array ? element.EnumerateArray().Take(100).ToArray() : [];
    }
    public static string NestedText(JsonElement element, string parent, string property) => element.TryGetProperty(parent, out var value) && value.ValueKind == JsonValueKind.Object ? Text(value, property) : string.Empty;
}
