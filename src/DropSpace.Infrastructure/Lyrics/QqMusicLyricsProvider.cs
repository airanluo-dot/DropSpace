...
public sealed class QqMusicLyricsProvider(LyricsHttpClient http) : ILyricsProvider
{
    ...
    public async Task<LyricsDocument> QueryAsync(LyricsQuery query, CancellationToken cancellationToken)
    {
        foreach (var terms in LyricsMatcher.SearchTerms(query))
        {
            ...
            if (best is null || best.Score < 4 || string.IsNullOrWhiteSpace(songId)) continue;
            ...
            if (document.Lines.Count > 0)
                return document.Bind(query, best.Title, best.Artist, best.Album, best.Duration, best.Score, songId);
        }
