...
public sealed class NativeIslandActivityRuntime : IAsyncDisposable
{
    ...
    {
        if (!_mediaSettings.EnableMediaActivity || !snapshot.IsActive || snapshot.PlaybackState != MediaPlaybackState.Playing)
        {
            StopTimelineTimer();
            return;
        }

        _timelineTimer ??= new Timer(_ => OnTimelineTick(), null, TimeSpan.FromMilliseconds(250), TimeSpan.FromMilliseconds(250));
    }

    private void OnTimelineTick()
    {
        if (_disposed || !_mediaSettings.EnableMediaActivity) return;
        var snapshot = _media.Current;
        if (!snapshot.IsActive || snapshot.PlaybackState != MediaPlaybackState.Playing)
        {
            StopTimelineTimer();
            return;
        }
        ...
        var elapsed = now - snapshot.Timeline.LastUpdated;
        if (elapsed < TimeSpan.Zero) elapsed = TimeSpan.Zero;
        if (elapsed > TimeSpan.FromSeconds(5)) elapsed = TimeSpan.FromSeconds(5);
        var rate = Math.Clamp(snapshot.Timeline.PlaybackRate, 0.1, 4);
        var position = snapshot.Timeline.Position + TimeSpan.FromTicks((long)(elapsed.Ticks * rate));
        if (snapshot.Timeline.Duration > TimeSpan.Zero)
        {
            position = TimeSpan.FromTicks(Math.Clamp(position.Ticks, 0, snapshot.Timeline.Duration.Ticks));
        }

        PublishMedia(snapshot with
        {
            Timeline = snapshot.Timeline with { Position = position, LastUpdated = now },
        });
    }
