...
- `WH_MOUSE_LL`/`WH_KEYBOARD_LL` remain observation-only supporting signals. Mouse-down first records the source surface and, when available, an exact UI Automation/MSAA item. Observation no longer stops merely because the source is unknown. An exact Explorer/Desktop item plus `SM_CXDRAG`/`SM_CYDRAG` takes the fast path; an unknown or non-exact source that crosses the same threshold becomes a generic candidate requiring OLE verification. A documented accessibility drag-start is strong evidence even when its provider cannot be classified by process or window class.
...
- Smart mode: exact Explorer/Desktop items and strong accessibility events use fast paths; other sources may form generic threshold candidates whose short-lived local OLE probe verifies real file data. Classic mode remains available when legacy compatibility is more important than the edge-input trade-off.
