...
The Settings **Adjust Island Position…** action keeps only the selected visual window visible and no-activate, revokes native drop targets on all other monitor islands, captures only the fixed island host, and converts physical pointer deltas to monitor-local DIP from the clamped projection. It commits on release and rolls back to saved coordinates on Escape. The Smart observer remains available for the global Escape hook but suppresses candidate creation for the edit; Classic activation hosts are disabled until the edit ends. Runtime-only identities reject Custom placement and expose Automatic-only controls.
