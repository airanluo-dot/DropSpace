...
## Preview.21 local settings and action usability update

Settings apply only edited fields to the latest saved snapshot. Unchanged startup and device-host settings do not restart their services. Compensation retries only failed steps; failures identify their stage and restore visible control state. Overlay property notifications are differential, avoiding native reconfiguration for unrelated settings. Cross-device clipboard enables the required Windows device connection; disabling that connection also disables clipboard sync.
