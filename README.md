...
Experimental Preview: **v0.2.0-preview.3** repairs the remaining no-response path by combining the verified Explorer/Desktop press surface with documented `EVENT_OBJECT_DRAGSTART/CANCEL/COMPLETE` signals; exact UI Automation item hit-testing is no longer the only route to reveal. It also gives DragReady, Compact, Expanded, Dismissing, Dynamic Island, and Notch one DPI-aware Smart-mode anchor below Windows Drop Tray. Smart idle still leaves no top-edge activation HWND, the old top-edge target remains opt-in, and Stable-channel users do not receive this Preview.
