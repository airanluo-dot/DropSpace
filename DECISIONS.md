...
- The runtime chain is NetEase → BetterNCM → unchanged official InfLink-rs → Windows SMTC
  → existing DropSpace GSMTC services. DropSpace never calls `window.InfLinkApi`, vendors
  InfLink implementation, or maintains a fork. Enhancement deployment has separate services.
- Downloads are opt-in official GitHub release artifacts, size/time/redirect bounded.
  InfLink uses the official release asset SHA-256; the last BetterNCM 1.3.4 DLL uses reviewed
  pinned SHA-256 per architecture because that historical release has no published digest.
  Microsoft's VC runtime is downloaded only from Microsoft and verified with Authenticode
  and Microsoft publisher identity before silent installation. No new NuGet dependency.
- Installation receipts track exact file ownership, hashes and rollback backups. Existing
  unrelated plugins/configuration are preserved; unrecognized component files block replacement.
  A file's presence is never proof of enhancement. Complete metadata, actual timeline progress
  and Windows controls must be observed. Native completeness skips third-party installation.
- Standard shuffle/repeat state and commands belong to the generic media abstraction and
  are capability-driven for every player. NetEase compatibility checks stay outside it.
- BetterNCM 1.3.4 and InfLink 3.2.11 do not officially claim support for this host's NetEase
  3.1.40; target-machine verification and rollback are release requirements, not assumptions.
- Native testing confirms metadata/timeline enhancement, but InfLink 3.2.11 leaves initial
  modes null and its upstream handlers ignore requested Shuffle/Repeat values in favor of
  toggles. Do not modify upstream or substitute a weaker acceptance definition. Full success
  requires an upstream protocol fix or a separately authorized change to requirements.
- Beta26 publication remains paused under the user's latest instruction. See
  `docs/dev/netease-enhancement.md` for implementation evidence and remaining limits.
...
## D-053 — Static per-user shell intake and one recoverable Undo slot

- Date: 2026-08-28
- Status: Accepted for v0.3.0-preview.9
- Decision: Use an Installer-owned per-user static Explorer verb plus a per-user SendTo shortcut, both invoking the existing executable and typed `--shell-add` contract. Route direct and redirected activation through the current `AppInstance` owner without foreground activation. Keep Portable registration-free. Use one in-memory eight-second Undo slot for pin changes and pending-delete transactions for Remove/Clear, with schema 3 token/expiry columns and startup expiry recovery.
- Rationale: Static registration avoids an Explorer-loaded DLL, elevation, package-identity dependency, and stale Portable paths. A single shell/intake service keeps source handling consistent. Pending rows preserve exact DropSpace identity and metadata without duplicating source/payload bytes, while a single slot makes replacement/expiry behavior deterministic and bounded.
- Constraints: The static Explorer verb may be under Windows 11 **Show more options** for unpackaged installs. Command-line size, item count, Windows OS/DPI/provider behavior, installer lifecycle, and real multi-selection/focus behavior require hosted Windows/manual evidence. No path/content diagnostics, source-file deletion, or fake optional action is allowed.

## D-054 — Smart Drag visual ownership requires positive OLE file evidence

- Date: 2026-09-02
- Status: Accepted for v0.3.0-preview.10
- Decision: Pointer threshold, exact Explorer/Desktop item evidence, accessibility drag-start events, source-surface classification, and the existence of an OLE drag loop may create only an invisible Smart candidate. Every candidate passes through the one bounded query-only probe, and the Dynamic Island becomes visible only after `CF_HDROP`, a Shell Item array, or the `FileGroupDescriptorW` + `FileContents` descriptor pair is classified as file-like. The probe remains hollow, no-activate, tool-window, topmost, `DROPEFFECT_NONE`, and short-lived. Rejected, timed-out, late, duplicate, or stale callbacks cannot reveal or reopen a session.
- Rationale: Strong intent and source attribution identify a possible drag but do not prove that its payload is a file or folder. Requiring the same positive OLE authority for every candidate removes false Smart reveals without introducing a permanent hit surface or input interception.
- Constraints: Smart remains source-agnostic and observation-only; it never reads payload content during verification, logs paths or filenames, injects input, suppresses the source, or falls back to the Classic top-edge target. Real provider, cursor-feedback, mixed-DPI, and Windows 10/11 evidence remain release gates.

## D-055 — Preview.13 unifies readable content and keeps async/native boundaries explicit

- Date: 2026-09-03
- Status: Accepted for v0.3.0-preview.13
- Decision: Preview, image transforms, hash, ZIP, capability profiles, and image-related actions resolve content through one `IItemContentResolver`. The resolver recognizes semantic image content from either an external file path or a confined app-owned payload, verifies availability, and derives bounded extension/MIME metadata without exposing payload paths in diagnostics. Smart OLE visual authorization is exactly `CanAcceptNow || CanMaterialize`; evidence-only classifications remain hidden, `CF_HDROP` is resolved to a non-empty path set before acceptance, and both completion and cleanup messages are dispatched through the probe owner thread. Main-page actions use the current ListView selection only when the clicked card is selected; otherwise they execute against the clicked card alone. Global hotkey lifecycle is asynchronous and bounded. DropLink replay protection checks the stored peer secret before reserving a nonce and keeps total/per-peer cache bounds. Manual unsigned Preview installation remains an explicit user action; unattended installation still requires a trusted publisher.
- Rationale: The Preview.12 audit found that repository-shaped image records could not reach Preview or Quick Actions, OLE evidence could reveal a target before the real receiver accepted it, and several native/network lifetimes had either stale cleanup or unbounded admission paths. These changes preserve the local-first source boundary while making content custody, visual ownership, selection semantics, and shutdown behavior testable.
- Constraints: WinUI/WinRT image transforms, real OLE provider behavior, Windows 10/11/DPI/manual multi-selection evidence, and live DropLink/network/browser acceptance still require the Windows CI/manual matrix. Hosted Linux cannot prove those runtime rows.


## D-056 — Explicit mutation and asynchronous resource ownership

- Date: 2026-09-07
- Status: Accepted for Preview.17
