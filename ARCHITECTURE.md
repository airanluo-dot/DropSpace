...
- `DragSessionDetector` is the default experimental candidate detector. A dedicated message thread subscribes to UI Automation DragStart/DragCancel/DragComplete signals and installs read-only low-level mouse/keyboard observers. Hook callbacks only enqueue bounded records and always call the next hook. Explorer/Desktop source classification plus `SM_CXDRAG`/`SM_CYDRAG` prevents simple clicks, stationary presses, title-bar drags and unknown applications from revealing the Overlay. Duplicate signals are coalesced into one timed session.
- Candidate detection never accepts content. It asks `OverlayWindowService` to reveal the shaped visual window on the pointer's monitor, about 76 physical pixels below the screen edge. That window registers `IDropTarget` only while visible; OLE still requires a real `CF_HDROP`, returns `DROPEFFECT_NONE` for invalid data, and converges on `MainViewModel.AddPathsAsync` without moving source files. Hidden revokes the OLE target, applies an empty region and calls `SW_HIDE`.
...
The Preview intentionally chooses false negatives over false positives. Explorer/Desktop file-view classification and system drag thresholds are the bounded fallback because UI Automation providers are not required to raise every Drag event. Third-party applications and virtual-file providers are not inferred from mouse movement alone. UIA/mouse signals only reveal a temporary target; `CF_HDROP` is the final content boundary. Escape, pointer release, UIA cancel/complete, OLE completion and a 30-second watchdog all converge on one idempotent session close. No Explorer injection, Shell modification, undocumented Drop Tray state, input suppression, cursor polling, permanent render loop or source-path diagnostic is used.

Preview.2 corrects the candidate evidence boundary: `ProcessSignalsAsync` can resume on any pool thread, so the synchronous UIA inspection brackets each call with COM initialization on that exact thread. `ElementFromPoint` is treated as a leaf and a raw-view walker inspects at most eight ancestors for the enclosing file item. COM objects are released within the same inspection. Recognition/rejection/COM-failure counters are path-free diagnostics.
