...
- `Strings/en-US/Resources.resw` is the complete base resource set; `Strings/zh-CN/Resources.resw` has an exact matching key set. XAML keeps its `x:Uid` metadata and mirrors it to `XamlResourceOverride`, which reapplies every supported property and automation name from the same explicit localizer context; imperative UI, service status, error prompts, and update feedback use the narrow Core `IAppStringLocalizer` contract.
