...
- `OleDragDropService` creates a zero-alpha native activation HWND and registers a managed `IDropTarget` with `RegisterDragDrop`. Idle it exposes only a 960-DIP-wide, one-physical-pixel top hot edge with `HTCLIENT`; after `DragEnter` the same HWND expands to 760 × 112 DIP, remains topmost and owns that OLE operation through `Drop`/`DragLeave`, then contracts. The visible Overlay has a separate target for direct drops while Compact/Expanded. Both converge on `MainViewModel.AddPathsAsync`.
