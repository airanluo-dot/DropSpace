...
- `DragSessionDetector` is the default Smart candidate detector. A dedicated message thread registers documented out-of-context `EVENT_OBJECT_DRAGSTART/CANCEL/COMPLETE`, provider-supplied `EVENT_SYSTEM_DRAGDROPSTART/END`, and read-only low-level mouse/keyboard observers. Registration completes before the thread enters `GetMessage`; no root-wide accessibility call can block the pump. Callbacks only enqueue bounded records and always preserve source input. The serialized worker keeps Explorer/Desktop exact-item evidence as the fast path, accepts a real accessibility drag-start as strong evidence even when the provider is unknown, and keeps observing an unknown press until it crosses `SM_CXDRAG`/`SM_CYDRAG`. Duplicate signals and stale callbacks converge through one session-ID policy.
- `OverlayWindowService` begins speculative reveal as soon as a session starts. A generic threshold candidate simultaneously creates one `EphemeralOleDragProbe`: a 144-physical-pixel, 1/255-alpha, `NOACTIVATE|TOOLWINDOW|TOPMOST` popup with a real 12-pixel center Region hole and a 60 ms hard lifetime. It is absent at idle, always returns `DROPEFFECT_NONE`, never takes focus, and posts completion so `RevokeDragDrop`/`DestroyWindow` run outside `IDropTarget::DragEnter`. Verification reject/timeout reverses only the matching session; a newer session cannot be cancelled by old work.
- `OleFileDataClassifier` is the single query/parse authority for the probe, Classic host, and visible Island. It accepts bounded `CF_HDROP` paths and Shell IDLists that resolve to file-system paths; `FileGroupDescriptorW` + `FileContents` is classified as virtual-file evidence without reading content. Preview.1 deliberately fails closed for virtual-only Drop because bounded staging materialization is not yet implemented. Accepted paths converge on `MainViewModel.AddPathsAsync` without moving source files. Hidden revokes the visible OLE target, applies an empty region and calls `SW_HIDE`.
