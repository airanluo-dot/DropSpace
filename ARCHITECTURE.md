...
- `DragSessionDetector` is the default experimental candidate detector. A dedicated message thread registers documented out-of-context `EVENT_OBJECT_DRAGSTART/CANCEL/COMPLETE` events, supplemental UI Automation drag signals, and read-only low-level mouse/keyboard observers. Callbacks only enqueue bounded records and always preserve source input. The serialized worker distinguishes a known Explorer/Desktop file-view surface from exact UIA item evidence: the accessibility drag event can promote the former, while movement-only fallback requires the latter plus `SM_CXDRAG`/`SM_CYDRAG`. Duplicate signals are coalesced into one timed session.
