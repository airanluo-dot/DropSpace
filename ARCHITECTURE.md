...
- `DragSessionDetector` is the default Smart candidate detector. A dedicated message thread registers documented out-of-context `EVENT_OBJECT_DRAGSTART/CANCEL/COMPLETE`, provider-supplied `EVENT_SYSTEM_DRAGDROPSTART/END`, and read-only low-level mouse/keyboard observers. Registration completes before the thread enters `GetMessage`; no root-wide accessibility call can block the pump. Callbacks only enqueue bounded records and always preserve source input. The serialized worker distinguishes a verified Explorer/Desktop root and exact UI Automation/MSAA item evidence: a real accessibility drag event can promote the former, while movement-only fallback requires the latter plus `SM_CXDRAG`/`SM_CYDRAG`. Duplicate signals are coalesced into one timed session.
- Candidate detection never accepts content. It asks `OverlayWindowService` to reveal the shaped visual window on the pointer's monitor. `OverlayPlacementPolicy` supplies one DPI-aware anchor about 76 physical pixels below the screen edge to every Smart-mode state and both visual modes; the host is sized for the offset Expanded surface. That window registers `IDropTarget` only while visible; OLE still requires a real `CF_HDROP`, returns `DROPEFFECT_NONE` for invalid data, and converges on `MainViewModel.AddPathsAsync` without moving source files. Hidden revokes the OLE target, applies an empty region and calls `SW_HIDE`.
...
- `OverlayMotionController` continuously integrates Dynamic Island width, height, offset, radii, opacity, content reveal, and drop feedback toward replaceable spring targets. `CompositionTarget.Rendering` is attached only while a target is unsettled and removed at rest. There is no alternate Notch display mode or display-mode transition state.
...
- `Strings/en-US/Resources.resw` is the complete base resource set; `Strings/zh-CN/Resources.resw` has an exact matching key set. Dependency-object XAML keeps its `x:Uid` metadata and mirrors it to `XamlResourceOverride`, which reapplies every supported property and automation name from the same explicit localizer context; `Window` roots use the same override directly after XAML initialization because they are not dependency objects. Imperative UI, service status, error prompts, and update feedback use the narrow Core `IAppStringLocalizer` contract.
...
## v0.2.0 Smart drag evolution

Smart mode intentionally chooses false negatives over false positives. Explorer/Desktop file-item classification and system drag thresholds are the bounded fallback because accessibility providers are not required to raise every Drag event. Third-party applications and virtual-file providers are not inferred from mouse movement alone. Accessibility/mouse signals only reveal a temporary target; `CF_HDROP` is the final content boundary. Escape, pointer release, accessibility completion, OLE completion and a 30-second watchdog all converge on one idempotent session close. No Explorer injection, Shell modification, undocumented Drop Tray state, input suppression, cursor polling, permanent render loop or source-path diagnostic is used.
