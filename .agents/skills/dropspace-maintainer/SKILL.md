...
- Keep lossy pointer movement separate from a bounded, diagnosed critical lane for press/release/cancel/completion/probe signals, while preserving timestamp order across both lanes. Pointer release enters completion grace, a fresh pointer-down supersedes immediately, and every probe/visual callback is session-gated.
