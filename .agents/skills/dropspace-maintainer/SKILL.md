...
- Use one bounded `OleFileDataClassifier` across probe, Classic and visible targets. Query formats only during verification. Keep `IsFileLikeEvidence`, `CanAcceptNow`, and `CanMaterialize` separate; accessibility proves drag intent, never file payload. Prefer `SHCreateShellItemArrayFromDataObject`, retain bounded CIDA only as fallback, and materialize virtual descriptors only after real Drop through confined, cancellable, rollback-safe streaming.
- Keep lossy pointer movement separate from an unbounded reliable critical lane for press/release/cancel/completion/probe signals, while preserving timestamp order across both lanes. PointerMoved is the only lossy signal; critical write failures are shutdown diagnostics, not normal capacity pressure. Pointer release enters completion grace, a fresh pointer-down supersedes immediately, and every probe/visual callback is session-gated.
- Probe cleanup must retain all three layers: owner `PostMessage`, owner work queue, and forced watchdog. Every path revokes OLE, destroys the HWND, removes registry ownership, and disposes timers.
- Treat the existing Dynamic Island as the Quick Panel. Preserve default configurable `Win+Shift+Space`, file/image/text/URL drag-out, manual/Share intake, Drop Batch metadata/actions, explicit best-effort exclusions, and schema-9 per-monitor `Automatic`/`Custom` DIP coordinates keyed by stable DisplayConfig-derived monitor identity. Clamp only the runtime projection; never overwrite the saved coordinates. The settings-page Adjust Position action is transient and no-activate: it converts physical pointer deltas to monitor-local DIP, suppresses Smart Drag candidate creation, commits once on release, and rolls back on Escape.
- `MonitorDescriptor.Id` is the persistent display identity used by settings; its HMONITOR `Handle` is process-lifetime runtime state. `DisplayIdentityService` owns DisplayConfig target-path mapping and marks runtime fallback IDs non-persistent. Legacy schema-8 placement keys are mapped only when they match a currently enumerated handle; unresolved keys must not block startup.
