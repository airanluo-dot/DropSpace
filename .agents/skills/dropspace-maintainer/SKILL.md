...
- Preserve the portable resource-index build: stage `Strings`, XAML, and assets while excluding `Package.appxmanifest`, so MakePri creates the unpackaged `Application` root without starving WinUI's XAML/file-resource lookup; generate one packaging-free `resources.pri`, bundle it into the self-extracting single EXE, and use an explicit resource context for imperative lookup.
