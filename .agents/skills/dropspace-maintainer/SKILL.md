...
## v0.3.0-preview.16 durable product facts

- Preview.16 hardens the existing Windows 10 1809/Windows 11 Dynamic Island slice: encrypted-share QR payloads render in memory, Smart Drag suppression preserves terminal convergence, and non-async OLE sources are copied into DropSpace-owned staging before the source lifetime ends.
- Cross-device Clipboard, Device Handoff, and encrypted-share revoke recovery commit enabled state only after owned resources are established; failed initialization unwinds partial state and remains retryable. Settings changes use reverse-order, per-step best-effort rollback while preserving the original forward exception.
- Native clipboard and hotkey subscribers are isolated from Win32 callbacks; image ingress and preview decode use platform metadata plus the shared decoded-memory/pixel budget; PDF uses the platform page count; QR capability is bounded; media remains candidate-only with graceful fallback.
- Nearby receiver admission, DropLink host lifecycle, DNS-SD registration, and local private-IPv4 selection are serialized and bounded. DropLink verification/finalization and temporary output cleanup have finite lifetimes. Revoke storage prunes on save while preserving the newly created handle, and shared update operations expose per-caller cancellation waits.
- Preview.16 remains CONDITIONAL until Windows CI/release packaging, real Windows OS/DPI/OLE/provider/two-device/Worker/browser acceptance, public GitHub Release assets, website/API synchronization, and the final execution report are evidenced. Never infer those rows from source inspection or hosted tests alone.


## Preview.17 architecture audit invariants

- Preserve the App/Core/Infrastructure project graph and platform thread affinity. Database writes from item and transfer repositories share the owning SqliteDatabase write gate; failed initialization disposes the connection. No schema change accompanies this audit.
- Projection failure must not advance the applied revision and must remain retryable without an automatic failure loop. Main query revisions reject stale results after search/navigation; Space totals come from repository COUNT, not the bounded visible page.
- JsonSettingsService.UpdateAsync reads, mutates, validates and writes under one gate. Preserve concurrent runtime Pause and monotonic update-check fields when applying settings snapshots.
- App, UpdateService and Undo shutdown callers share an awaited completion task. Stop admitting work, cancel owned operations, drain them, and only then release owned resources. A shared update operation still outlives any individual caller's canceled wait. Overlay projection disposal is asynchronous.
- Automatic cross-device clipboard propagation uses one consumer and a 16-entry DropOldest queue; disable unsubscribes and drains it. Automatic image reads are bounded at 10 MiB; manual reads at 50 MiB. Recheck enabled, pause and peer state before sending. Manual sends remain caller-owned; HTTP receives remain host-owned.
- StagedFileImportService owns staging admission, bounded payload copy, database commit and cleanup. Reject paths outside staging and reparse traversal, never delete external source files, and clean the entire admitted batch after cancellation, including unvisited staging files.
- Preview cache is derived and disposable: 64 entries, 64 MiB total, 16 MiB serialized entry, 24-hour age. External source references bypass it. Generation-gated writes prevent delayed work from recreating data after Clear; failed clear disables reads until retry. Cache IO failure cannot hide a valid preview. Undo finalization/recovery, clear-history and retention invalidate cached content.
- Reproduce the all-tracked-file inventory with `python scripts/audit-repository.py --revision HEAD --output audit.json`. Hashes and regex hints are coverage aids, not semantic or runtime proof. The detailed audit and validation boundaries live under `docs/audit/2026-09-07/`.
- Preview.17 remains CONDITIONAL for real Windows OS/DPI/monitor/OLE/accessibility, two-device network and deployed Worker/browser acceptance. Hosted tests, structural resource bounds and a successful release do not prove those cells or measured performance improvements.

## Current settings and action reliability facts

- Settings forms must merge only fields changed by the active form into the latest persisted `AppSettings` snapshot. Preserve runtime-owned pause and update-check fields, serialize the transaction, and avoid initializing unchanged optional startup/device services as a side effect of a local preference edit.
- Settings compensation is reverse-order and per-step. A failed update must expose a bounded stage/correlation identifier, restore visible state on the UI dispatcher, and retry only failed compensation steps; do not mask the original failure or retry unrelated network initialization.
- Quick Actions are one registry projection shared by Main and Overlay. Image Automatic profiles prefer resize, format conversion, and metadata removal. Safe hash/ZIP/QR/metadata exports may use the private default output directory immediately; exports remain collision-safe and source-preserving.
- The item-action surface must keep core actions understandable with accessible labeled controls. SHA-256 results expose selectable checksum text and copy support. Image resize/conversion offers bounded original/percentage presets before custom pixels and reports the resulting dimensions.
- These usability changes do not prove Windows startup registration, WinUI thread affinity, image codec behavior, DPI/accessibility, physical two-device/network, deployed Worker/browser, or long-run acceptance. Keep those rows conditional until target evidence exists.

## v0.3.0-preview.22 durable product facts

- Preview.22 is the Native Island integration Preview. The minimum supported Windows build is 20348 across `Directory.Build.props`, Core compatibility policy, app target, MSIX/identity manifests, Inno Setup, updater metadata, README, compatibility report, and website source.
- `IslandActivityRouter` is the only activity arbitration boundary for the existing Dynamic Island. Lower numeric priority wins: Drop, manual expansion, notification, volume, media, then widget idle. Activities are source-replaceable, expiry-aware, and must never create a second overlay surface.
- SMTC media integration is optional and fail-closed. Metadata, artwork, playback/timeline state, progress, play/pause, previous, next, and seek are bounded and owned by `WindowsMediaSessionService`; unavailable SMTC must not block startup or Smart Drag/OLE.
- Timed lyrics support normalized queries, bounded online/local lookup, LRC multiple timestamps, delay, secondary/word-sync model fields, and cancellation. Providers remain capability/configuration inputs; lyrics lookup must never block app startup or settings commit.
- Spectrum is sampled only while media is actively playing. Process loopback is the preferred capture contract; default-render endpoint metering is an explicit bounded fallback when a single media process cannot be resolved. Pause/disable must stop the timer and release COM resources.
- Notification listener access is opt-in and fail-closed. Volume integration observes the default render endpoint only; it must not replace the Windows volume flyout, install a keyboard hook, or claim ownership of system volume UI.
- Clock, Calendar, Resource Usage, and Settings widgets use a normalized 6×3 layout model and compact projection. New native-activity settings are schema 12, merge through `SettingsChangePolicy`, and roll back through `SettingsApplicationCoordinator`.
- Preview.22 is CONDITIONAL until Windows build 20348/Windows 11 media-player, LRC, notification-permission, volume, process-loopback/fallback, DPI, monitor, accessibility, and long-run evidence is recorded. Hosted CI and source inspection do not substitute for those rows. The five public release assets and official website/API synchronization remain mandatory.

## v0.3.0-preview.23 durable product facts

- Preview.23 is the Native Island UI refactor Preview. The Dynamic Island remains one native HWND: DWM owns the capability-gated outer backdrop/frame and XAML owns the transparent inner rounded clip. Smart Drag/OLE ownership, no-focus popup behavior, and the Windows system-volume flyout remain intact; no second overlay surface is introduced.
- Compact media uses the bounded `CompactMediaLayoutCalculator`: artwork, lyric/title/source fallback, optional secondary lyrics, and six spectrum bars are measured into a fixed-height, overflow-safe dynamic width. Spectrum capture is active-media-only; paused/no-session state follows `IdleHidePolicy` with a default 3-second delay and a bounded 0.5–30 second setting range.
- Expanded navigation is a single non-wrapping `ExpandedIslandPager` in Files → Music → Widgets order. Files uses recent staged items and an empty Open DropSpace state; Music exposes cover/metadata/lyrics/spectrum/progress/controls; Widgets renders the normalized 6×3 Clock/Calendar/Resource/Settings grid and compact slots.
- Main promotes Music and the planned Settings categories (General, Island, Widgets, System Activities, Devices & Sharing, Updates, About). Media source allowlists and schema-13 nested settings are persisted and validated; `LyricsFrame` revisions keep lyric updates ordered. Plugins, custom fonts, native volume-flyout replacement, and Liquid Glass remain outside the product boundary.
- Preview.23 is published as a prerelease with exactly five public assets (`DropSpaceSetup.exe`, `DropSpace.exe`, `DropSpace-x64.msix`, `SHA256SUMS.txt`, `update-manifest.json`). The official website/API synchronization and live release verification are required; physical Windows media/notification/audio/DPI/OLE/accessibility/provider/long-run evidence remains CONDITIONAL until recorded on target hosts.
